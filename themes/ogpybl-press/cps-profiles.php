...
get_header();
$g365_ad_info = ogp_start_ads( $post->ID );
$print_search = true;
...
if( !is_string($stat_data) ) :
  //img defaults
  $default_profile_img = get_site_url() . '/wp-content/uploads/ogp_profile_placeholder.gif';
...
?>
<section id="content" class="grid-x grid-margin-x site-main small-padding-top medium-padding-bottom profile-wrap cps-page<?php if ( $g365_ad_info['go'] ) echo $g365_ad_info['ad_section_class']; ?>" role="main">
          ...
          <div class="grid-x grid-margin-x">
            <div class="cell small-12">
              ...
              <?php foreach( $position_order as $dex => $position ) {
                ...
                <a class="button small-margin-left<?php echo (empty($positions->{$position})) ? ' disabled' : '' ; ?>" href="#<?php echo strtolower(preg_replace("([^a-zA-Z])", "-", preg_replace("([^a-zA-Z -])", "", $position))); ?>_heading<?php echo '_' . $class; ?>"><?php echo $position; ?></a>
              <?php } ?>
              </div>
            </div>
            <?php
            ...
            foreach( $position_order as $dex => $position ) { //start position
              ...
              ?>
              <div class="cell small-12 gset large-margin-top">
              ...
              <div class="cell small-12 grid-x grid-margin-x small-up-1 medium-up-2 small-margin-bottom medium-padding cps__player-container">
                <?php
                foreach( $player_stats as $dex => &$player_stat ) { //start player
                  ...
                  <div class="cell tiny-padding" id="<?php echo $player_stat->id; ?>_player">
                    <div id="profile-wrapper" class="grid-x grid-margin-x flex-align-content-start small-padding-sides medium-padding-top medium-padding-bottom maximum-height gset profile border-radius">
                      <div class="cell small-12">
                        <h1 class="profile-name small-profile cell auto">
                          <?php echo $player_stat->name; ?>
                        </h1>
                      </div>
                      <div id="profile-image" class="cell small-6 small-offset-3 medium-offset-0 cps__profile-image">
                          ...
                          <table class="text-center ghost-white-bg no-margin-bottom">
                            <tbody>
                              <tr>
                                ...
                                <?php $title = str_replace(array('Video Link'), array('Video'), $event_data[$player_stat->event]->stats->{$stat_handle}->name); ?>
                                <th>Highlight Reel!</th>
                                <?php endforeach; ?> 
                              </tr>
                              <tr>
                                <?php foreach($player_stat->stats as $stat_handle => $stat_value) : ?>
                                <td> 
                                  <a href="<?php echo $stat_value; ?>"> <?php echo $stat_value; ?></a>
                                  <?php $embedded = str_replace(array('https://www.youtube.com/watch?v='), array('https://www.youtube.com/embed/'), $stat_value); ?>
<!--                                   <iframe width="300" height="215" src="<?php echo $embedded; ?>"></iframe> -->
