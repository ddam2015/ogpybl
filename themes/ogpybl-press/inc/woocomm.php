...
//         $script_url = (strpos(site_url(), 'dev.') === false)
//       ? 'https://grassroots365.com/data-processor.js'
//       : 'https://dev.grassroots365.com/data-processor.js';
    
//     wp_enqueue_script('js-g365-all', $script_url, array('jquery'), '695450', true);
...
function g365_add_event_id_reference() {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/ogp-press/css/', 'https' ) . "style-admin.css?ver=2.4" );
...
function ogp_cart_process(){
  ...
  if( !empty(WC()->cart->get_cart()) ) {
    ...
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
      ...
      $extra_types = array( 'fee', 'playerclub', 'gear', 'event', 'findus', 'highsch', 'ybl', 'coach', 'camps' );
            ...
            case 'playerclub':
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'text',
                  'placeholder'=> 'Players Height - Feet',
                  'name'  => 'player_height_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'text',
                  'placeholder'=> 'Are you a returning Player?',
                  'name'  => 'returning_player_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'returning-player'),
                );
              }
              break; 
            ...
            case 'highsch':
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'text',
                  'placeholder'=> 'Player '. $i . ' Full Name ',
                  'label' => 'Player Name ' . $i,
                  'name'  => 'player_name_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug)
...
//                 );
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'text',
                  'placeholder'=> 'Coach Insurance Company',
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'text',
                  'placeholder'=> 'Coach Insurance Policy Number',
                  'name'  => 'insurance_policy_number_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'text',
                  'placeholder'=> 'Special Request/Comments?',
                  'name'  => 'spceial_requests_comments_' . $i,
                  'required'=> false,
                  'class' => array($product_cat_slug)
                );
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'select',
                  'placeholder'=> 'Ever convicted of a crime of violence, crime against a person, or a felony?',
                  'name'  => 'felony_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                  'options' => array(
                    '' => 'Ever convicted of a crime of violence, crime against a person, or a felony?',
                    'no' => 'No',
                    'yes' => 'Yes'
                  )
                );
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'text',
                  'placeholder'=> 'Uniform Colors',
                  'name'  => 'uniform_colors_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'select',
                  'placeholder'=> 'What Division are you coaching?',
                  'name'  => 'division_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                  'options' => array(
                    '' => 'What Division are you coaching?',
                    'freshman' => 'Freshman (1st/2nd)',
                    'jv' => 'JV (3rd/4th)',
                    'varsity' => 'Varsity (5th/6th)',
                    'varsity_senior' => ' Varsity Senior (7th/8th)'
                  )
                );
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'select',
                  'placeholder'=> 'What type of team are you coaching?',
                  'name'  => 'team_gender_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                  'options' => array(
                    '' => 'What type of team are you coaching?',
                    'boys' => 'Boys',
                    'girls' => 'Girls',
                    'coed' => 'Coed'
                  )
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'select',
                  'placeholder'=> 'What level is your team?',
                  'name'  => 'team_level_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                  'options' => array(
                    ''=> 'What level is your team?',
                    'bronze' => 'Beginner/Bronze',
                ...
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'type'  => 'select',
                  'placeholder'=> 'Will you need players from the free agent pool?',
                  'name'  => 'need_free_agent_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                  'options' => array(
                    '' => 'Will you need players from the free agent pool?',
                    'Yes' => 'Yes',
                    'No' => 'No'
                  )
                );
...
add_action( 'woocommerce_admin_order_data_after_billing_address_export_csv', 'ogp_player_checkout_field_display_admin_order_meta_export_csv', 10, 1 );
function ogp_player_checkout_field_display_admin_order_meta_export_csv($order){
  //parse the field names, if we have them, then echo them
  $fields = get_post_meta( $order->id, 'ogp_extra_data', true );
...
//     custom start
    ...
    foreach($post_ids as $order){
      ...
      $fields = get_post_meta( $order, 'ogp_extra_data', true );
...
function ogp_user_fields_display( $user ) {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/ogp-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
  wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '695450', true );
  ?>
